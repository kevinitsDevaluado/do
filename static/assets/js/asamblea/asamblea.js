    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (let i = 0; i < response.length; i++) {
                ...
                fila += '<td> <a href="/media/'+response[i]["fields"]['archivo_asa']+'" class="status-tb-btn rounded-circle btn-primary me-4" target="_blank_"><i class="fas fa-eye"></i></a>';
                fila += '<button type = "button" class="status-tb-btn rounded-circle btn-primary me-4"';
                fila += ' onclick = "abrir_modal_edicion(\'/indexadmin/editarasamblea/' + response[i]['pk'] + '/\');"><i class="fas fa-pencil"></i></button>';
                fila += '<button type = "button" class="status-tb-btn rounded-circle btn-danger" ';
                fila += 'onclick = "abrir_modal_eliminacion(\'/indexadmin/eliminarasamblea/' + response[i]['pk'] + '/\');"><i class="fas fa-trash-alt"></i></buttton></td>';
                fila += '</tr>';
