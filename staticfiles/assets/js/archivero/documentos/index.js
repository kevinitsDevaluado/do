    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (let i = 0; i < response.length; i++) {
                ...
                fila += '<td> <a href="/media/'+ response[i]["fields"]['documento_doc']+'" class="text-dark text-center" target="_blank_"><i class="bi bi-file-earmark-pdf iconn"></i> Visualizar </a>'+ '</td>';             
                fila += '<td  style="display:none">' + response[i]["fields"]['contenido_doc'] + '</td>'; 
                fila += '<td><button type = "button" class="status-tb-btn btn-primary me-4"';
                fila += ' onclick = "abrir_modal_edicion(\'/archivero/actualizardocumentos/' + response[i]['pk'] + '/\');"><i class="fa-solid fa-arrows-rotate"></i></button>';
                fila += '<button type = "button" class="status-tb-btn btn-danger" ';
                fila += 'onclick = "abrir_modal_eliminacion(\'/archivero/eliminardocumentos/' + response[i]['pk'] + '/\');"><i class="fa-solid fa-trash"></i></buttton></td>';
                fila += '</tr>';
